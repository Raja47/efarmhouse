...
class PayPalService
{
    ...
    {   
        ...
        $shipping = sprintf('%0.2f', \Cart::getCondition('UPS SHIPPING CHARGES') ? \Cart::getCondition('UPS SHIPPING CHARGES')->getCalculatedValue($subTotal) : 0 );
        // Add any tax amount if you want to apply any tax rule
        $tax = sprintf('%0.2f', \Cart::getCondition('Newyork Tax') ? \Cart::getCondition('Newyork Tax')->getCalculatedValue($subTotal) : 0 );
        ...
        foreach ($order->items as $item)
        {
            ...
            $rawSubTotal += ( (float) sprintf('%0.2f', $item->price) ) * $item->quantity;   
        ...
        $details->setShipping($shipping)
                ->setTax($tax)
                ->setSubtotal(sprintf('%0.2f', $rawSubTotal));

        $rawTotal = $rawSubTotal + ((float) $shipping) + ((float) $tax); 
        $amount = new Amount();
        $amount->setCurrency(config('settings.currency_code'))
                ->setTotal(sprintf('%0.2f', $rawTotal))
                ->setDetails($details);
